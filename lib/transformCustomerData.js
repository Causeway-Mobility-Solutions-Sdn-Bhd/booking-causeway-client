...
export const transformCustomerData = (data) => {
  ...
  const reverseTransformDate = (dateString) => {
    ...
    try {
      ...
      console.error("Error reverse transforming date:", error);
      return "";
    }
  };
  ...
  const phoneCountryCode = phone.startsWith("+")
    ? phone.match(/^\+[0-9]+/)?.[0] || "+60"
    : "+60";

  const emergencyPhoneCountryCode = emergencyPhone.startsWith("+")
    ? emergencyPhone.match(/^\+[0-9]+/)?.[0] || "+60"
    : "+60";

  // Remove country code from phone numbers to get just the number part
  const phoneNumber = phone.replace(phoneCountryCode, "");
  const emergencyPhoneNumber = emergencyPhone.replace(
